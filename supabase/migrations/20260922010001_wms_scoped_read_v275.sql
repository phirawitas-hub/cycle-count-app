...
CREATE OR REPLACE FUNCTION public.get_wms_read_bundle_v275(p_session_id uuid, p_tab text DEFAULT 'wms'::text, p_since_revision bigint DEFAULT NULL::bigint, p_since_catalog bigint DEFAULT NULL::bigint, p_location_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SECURITY INVOKER
...
BEGIN
  ...
  END IF;
  IF p_tab IS DISTINCT FROM 'wms' OR p_session_id IS NULL THEN
  ...
  IF p_location_id IS NULL AND p_since_revision=clock_revision AND p_since_catalog=catalog_revision THEN
    RETURN jsonb_build_object('read_contract','wms-candidates-v1','schema_version',1,'actor_id',actor_id,'session_id',p_session_id,'tab',p_tab,
  ...
  WITH all_scope AS MATERIALIZED (
  ...
  ), approval_assignments AS MATERIALIZED (
    SELECT DISTINCT v.assignment_id FROM public.variance_approvals v
    JOIN all_scope a ON a.id=v.assignment_id
  ), recovery_guard_locations AS MATERIALIZED (
    -- Preserve the legacy global recovery gate, even when its zero-OH
    -- trigger belongs to otherwise completed work. Unknown baseline formats
    -- are retained, not reinterpreted using different SQL/JS numeric rules.
    SELECT s.location_id FROM public.stock_snapshot s
    WHERE s.session_id=p_session_id AND (s.onhand_qty IS NULL OR s.onhand_qty=0)
      AND (NOT delta OR s.location_id=ANY(changed_locations))
    UNION
    SELECT a.location_id FROM public.audit_log b JOIN all_scope a ON a.id=b.assignment_id
    WHERE b.session_id=p_session_id AND b.action='wms_import_count_baseline'
      AND (b.old_value IS NULL OR b.old_value !~ '^[0-9]*[1-9][0-9]*$')
  ), candidate_locations AS MATERIALIZED (
    -- Only completed work WITHOUT ANY approval is provably absent from WMS.
    -- Unknown states, drafts, zero-variance approvals and all historical
    -- decisions are retained. Keep the ENTIRE location for OH recovery.
    SELECT DISTINCT a.location_id FROM all_scope a
    LEFT JOIN approval_assignments v ON v.assignment_id=a.id
    WHERE p_location_id IS NOT NULL OR a.status IS DISTINCT FROM 'completed'
      OR v.assignment_id IS NOT NULL
      OR a.location_id IN (SELECT location_id FROM recovery_guard_locations)
  ), scope AS MATERIALIZED (
    SELECT a.* FROM all_scope a JOIN candidate_locations l ON l.location_id=a.location_id
    ...
    FROM public.stock_snapshot s WHERE s.session_id=p_session_id
      AND s.location_id IN (SELECT location_id FROM candidate_locations)
      AND (NOT delta OR s.location_id=ANY(changed_locations))
  ...
  SELECT jsonb_build_object(
    'read_contract','wms-candidates-v1','schema_version',1,'actor_id',actor_id,'session_id',p_session_id,'tab',p_tab,'unchanged',false,
...
REVOKE ALL ON FUNCTION public.get_wms_read_bundle_v275(uuid,text,bigint,bigint,uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_wms_read_bundle_v275(uuid,text,bigint,bigint,uuid) TO authenticated;
COMMENT ON FUNCTION public.get_wms_read_bundle_v275(uuid,text,bigint,bigint,uuid) IS
  'V2.75 WMS candidates: whole locations except completed-only work without approvals; same RLS, revision snapshot, formulas and historical records.';
